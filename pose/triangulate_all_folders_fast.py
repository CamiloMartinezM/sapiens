...
NUM_STEPS = 400  # Reduced from 1000
CONFIDENCE_EXPONENT = 2.0
REGULARIZATION_WEIGHT = 0.05  # Reduced

# Camera extrinsics optimization settings - Reduced for speed
CAMERA_LEARNING_RATE = 0.005  # Higher for faster convergence
CAMERA_STEPS = 600  # Reduced from 1500
CAMERA_REGULARIZATION_WEIGHT = 0.01

# RANSAC settings - Optimized for speed vs quality balance
RANSAC_ITERATIONS = 300  # Reduced from 500
...
def save_ply_file(points_3d, filepath):
    ...
    with open(filepath, 'w') as f:
        ...
        # Write vertices with colors (blue for all points)
        for point in valid_points:
            f.write(f"{point[0]:.6f} {point[1]:.6f} {point[2]:.6f} 100 150 255\n")

def save_optimized_camera_data(good_cameras, good_cams_sorted, good_cam_params_opt, good_intrinsics_list, folder_name):
    ...
    os.makedirs(os.path.dirname(OPTIMIZED_CAMERAS_FILE), exist_ok=True)
    with open(OPTIMIZED_CAMERAS_FILE, 'w') as f:
        json.dump(camera_data, f, indent=2)
    
    print(f"✓ Saved optimized camera data: {len(good_cams_sorted)} cameras")

def load_optimized_camera_data():
    """Load optimized camera data from previous run."""
    if not os.path.exists(OPTIMIZED_CAMERAS_FILE):
        return None
    
    try:
        with open(OPTIMIZED_CAMERAS_FILE, 'r') as f:
...
def rodrigues_to_rotation_matrix(rvec: np.ndarray) -> np.ndarray:
    """Convert a Rodrigues rotation vector to a 3×3 rotation matrix using NumPy."""
...
def rodrigues_to_rotation_matrix_torch(rvec: torch.Tensor) -> torch.Tensor:
    """Convert a batch of Rodrigues rotation vectors to 3×3 rotation matrices using PyTorch."""
    N = rvec.shape[0]
    theta = torch.linalg.norm(rvec, dim=1, keepdim=True)
    axis = rvec / (theta + 1e-12)

    K = torch.zeros((N, 3, 3), device=rvec.device)
    K[:, 0, 1] = -axis[:, 2]
    K[:, 1, 0] =  axis[:, 2]
    K[:, 0, 2] =  axis[:, 1]
    K[:, 2, 0] = -axis[:, 1]
    ...
    eye = torch.eye(3, device=rvec.device).expand(N, -1, -1)
    R = eye + torch.sin(theta) * K + (1 - torch.cos(theta)) * torch.bmm(K, K)
    return R

def rotation_matrix_to_rodrigues(R: np.ndarray) -> np.ndarray:
    """Convert a 3×3 rotation matrix to a Rodrigues rotation vector."""
    cos_theta = (np.trace(R) - 1.0) * 0.5
    cos_theta = np.clip(cos_theta, -1.0, 1.0)
    theta = np.arccos(cos_theta)
    if abs(theta) < 1e-12:
        return np.zeros(3)
    axis_skew = (R - R.T) / (2.0 * np.sin(theta))
    rx = axis_skew[2, 1]
    ry = axis_skew[0, 2]
    rz = axis_skew[1, 0]
...
def project_point(X: np.ndarray, cam_param: np.ndarray, intrinsics: tuple) -> np.ndarray:
...
def project_point_torch(X: torch.Tensor, cam_params: torch.Tensor, intrinsics: torch.Tensor) -> torch.Tensor:
    """Projects a batch of 3D points into pixel coordinates using PyTorch."""
    rvecs = cam_params[:, :3]
    tvecs = cam_params[:, 3:]
    R = rodrigues_to_rotation_matrix_torch(rvecs)

    X_cam = torch.bmm(R, X.unsqueeze(-1)).squeeze(-1) + tvecs
...
def triangulate_point_dlt(projection_matrices: list, points_2d: list) -> np.ndarray:
    """Performs linear (DLT) triangulation for a single 3D point."""
...
def robust_triangulation_with_ransac(intrinsics_list, extrinsics_list, projs, cam_idx_map, N_joints) -> np.ndarray:
    """Performs robust linear triangulation for each joint using RANSAC scheme to reject outliers."""
    pts3d = np.zeros((N_joints, 3), dtype=np.float64)

    projection_matrices = []
    for idx in range(len(intrinsics_list)):
        fx, fy, cx, cy = intrinsics_list[idx]
        R, t = extrinsics_list[idx]
    ...
    for j in range(N_joints):
        ...
        for _ in range(RANSAC_ITERATIONS):
            ...
            sample_indices = np.random.choice(len(views), size=2, replace=False, p=probs)
            ...
            try:
                ...
                for view in views:
                    ...
                    uv_reprojected = P @ X_4d
                    if uv_reprojected[2] == 0: continue
...
def process_single_folder_fast(folder_path, folder_name, optimize_extrinsics=False, is_first_folder=False):
    ...
    keypoints_dir = os.path.join(folder_path, 'landmarks')
    out_npy = os.path.join(folder_path, 'triangulated.npy')
    out_npy_initial = os.path.join(folder_path, 'triangulated_initial.npy')
    out_ply = os.path.join(folder_path, 'triangulated.ply')
    out_ply_initial = os.path.join(folder_path, 'triangulated_initial.ply')
    ...
    keypoint_files = glob.glob(os.path.join(keypoints_dir, '*.npy')) + glob.glob(os.path.join(keypoints_dir, '*.npz'))
    ...
    try:
        ...
        with open(transforms_path, 'r') as f:
            tf = json.load(f)
        fl_x, fl_y, cx, cy = float(tf['fl_x']), float(tf['fl_y']), float(tf['cx']), float(tf['cy'])
        img_w, img_h = int(tf.get('w', 2 * cx)), int(tf.get('h', 2 * cy))
        frames = tf['frames']
        ...
        for cam_id in sorted(cams_to_try):
            fp_npz = os.path.join(keypoints_dir, f"{cam_id:06d}.npz")
            fp_npy = os.path.join(keypoints_dir, f"{cam_id:06d}.npy")
            
            fp = fp_npz if os.path.exists(fp_npz) else (fp_npy if os.path.exists(fp_npy) else None)
            if fp is None:
                continue
        ...
        if use_optimized_cameras:
            ...
            extrinsics_list = []
            for i, cam_param in enumerate(optimized_camera_data['optimized_camera_params']):
                rvec = cam_param[:3]
                tvec = cam_param[3:]
                R = rodrigues_to_rotation_matrix(rvec)
                t = tvec
                extrinsics_list.append((R, t))
            ...
            intrinsics_list = [(fl_x, fl_y, cx, cy) for _ in range(C)]
            ...
            for cam_id in cams_sorted:
                ...
                R = M_w2c[:3, :3].astype(np.float64)
                t = M_w2c[:3, 3].astype(np.float64)
                extrinsics_list.append((R, t))

            cam_params_init = np.zeros((C, 6), dtype=np.float64)
            for idx, (R, t) in enumerate(extrinsics_list):
                rvec = rotation_matrix_to_rodrigues(R)
                cam_params_init[idx, :3] = rvec
                cam_params_init[idx, 3:] = t
        ...
        cam_params = torch.tensor(cam_params_init, dtype=torch.float32, device=device, requires_grad=False)
        ...
        meas_cam_indices = torch.tensor([m[0] for m in measurements], device=device, dtype=torch.long)
        meas_joint_indices = torch.tensor([m[1] for m in measurements], device=device, dtype=torch.long)
        meas_uv_obs = torch.tensor(np.array([m[2] for m in measurements]), dtype=torch.float32, device=device)
        meas_confidences = torch.tensor([m[3] for m in measurements], dtype=torch.float32, device=device)
        ...
        if optimize_extrinsics and is_first_folder:
            # Camera optimization
            pts3d.requires_grad_(False)
            cam_params.requires_grad_(True)
            cam_params_initial_tensor = torch.tensor(cam_params_init, dtype=torch.float32, device=device)
            
            optimizer_cams = torch.optim.Adam([cam_params], lr=CAMERA_LEARNING_RATE)
            
            for i in range(CAMERA_STEPS):
                ...
                if CAMERA_REGULARIZATION_WEIGHT > 0:
                    camera_regularization_loss = torch.nn.functional.mse_loss(cam_params, cam_params_initial_tensor)
                    total_loss = total_loss + CAMERA_REGULARIZATION_WEIGHT * camera_regularization_loss
...
def main():
    """Main function to process all folders in sub_10/pangrams-1."""
    ...
    parser.add_argument('--force_first_folder_optimization', action='store_true',
    ...
    for item in os.listdir(base_output_dir):
        ...
        if os.path.isdir(item_path) and item.isdigit() and len(item) == 6:
            all_folders.append((item_path, item))
    ...
    if args.optimize_extrinsics:
        ...
        if existing_data and not args.force_first_folder_optimization:
            ...
            if not parallel_mode or args.job_index == 1:
                first_folder_for_optimization = all_folders[0][1]  # Use the very first folder globally
                if args.force_first_folder_optimization and os.path.exists(OPTIMIZED_CAMERAS_FILE):
                    os.remove(OPTIMIZED_CAMERAS_FILE)
            else:
                ...
                while not os.path.exists(OPTIMIZED_CAMERAS_FILE) and (time.time() - wait_start) < max_wait_time:
                    time.sleep(30)
                if not os.path.exists(OPTIMIZED_CAMERAS_FILE):
                    print("ERROR: Camera optimization file not found after waiting. Job 1 may have failed.")
                    return
